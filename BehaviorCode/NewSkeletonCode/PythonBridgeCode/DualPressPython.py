...
def cbEndOfTrial():
    ...
    # consts
    biasNPts = constD['rewardStaircaseSlidingWinNTrials']
    biasMinTrs = constD['rewardStaircaseNTrialsBeforeStart']
    ...
    if constD['rewardStaircaseDoUseAllStimTrials']:
        stairNotCorrV = np.array(futureIn1d(varArr['outcomeList'], ['incorrect', 'ignore', 'dualrelease']), dtype='bool')
    else:
    ...
    if nTrs > biasMinTrs or np.isnan(computedBias):
        ...
        tWinLeftBias = constD['rewardStaircaseStartingLeftBias']  # will be a constant value if before min trials
    ...
    if constD['rewardStaircaseDoSlidingWin'] == 1:
        client.send_data(client.reverse_codec['tRewardRunningLeftBias'], tWinLeftBias)
