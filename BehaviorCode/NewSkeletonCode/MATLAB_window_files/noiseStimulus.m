function [retval] = noiseStimulus(data_struct, input)
...
if nSync >= 4,
  ...
  constsStruct = struct;
  constsStruct = ds.events;
  trialStruct = ds.events;
end
...
%% disp status
trStr = ['Noise stimulus- Trial ' num2str(trN) '- '];
stimStr = sprintf('size %d', round(input.grainDeg));
fprintf(1,'%s %s\n', ...
    trStr, stimStr);

%% run subfunctions
%input = exptRunSubfunctions(ds, input, { @plotNoiseStim });
