...
tTrialN = input.trialSinceReset;
  ...
  stimOn_mat = input.testStimOnTimeMs;
  az_mat = input.gratingAzimuthDeg;
  el_mat = input.gratingElevationDeg;
  dir_mat = unique(celleqel2mat_padded(input.tTestStimGratingDirectionDeg));
  diam_mat = input.gratingDiameterDeg; 
  sf_mat = input.gratingSpatialFreqCPD; 
  tf_mat = input.gratingTemporalFreqCPS;
  con_mat = unique(celleqel2mat_padded(input.tTestStimGratingContrast));
  phase_mat = input.gratingPhaseDeg;

  mask_dir_mat = unique(celleqel2mat_padded(input.tTestMaskGratingDirectionDeg));
  mask_con_mat = unique(celleqel2mat_padded(input.tTestMaskGratingContrast));
  ...
  tStr1 = ['Test Stim:'];
      ...
      tMaskStr1 = ['Mask Stim:'];
          tMaskStr1 = sprintf( [tMaskStr1 '\n',...
                 'Direction (deg): %s \n', ...
                 'Contrast (pct): %s \n'], ...
                        mat2str(mask_dir_mat), ...
                        mat2str(mask_con_mat.*100));
