...
tTrialN = input.trialSinceReset;
  ...
  az1_mat = unique(celleqel2mat_padded(input.tTargetOneGratingAzimuthDeg));
  el1_mat = unique(celleqel2mat_padded(input.tTargetOneGratingElevationDeg));
  dir1_mat = unique(celleqel2mat_padded(input.tTargetOneGratingDirectionDeg));
  diam1_mat = chop(unique(celleqel2mat_padded(input.tTargetOneGratingDiameterDeg)),2); 
  sf1_mat = unique(celleqel2mat_padded(input.tTargetOneGratingSpatialFreqCPD)); 
  tf1_mat = unique(celleqel2mat_padded(input.tTargetOneGratingTemporalFreqCPS));
  con1_mat = unique(celleqel2mat_padded(input.tTargetOneGratingContrast));
  phase1_mat = unique(celleqel2mat_padded(input.tTargetOneGratingPhaseDeg));

  dist1_dir_mat = unique(celleqel2mat_padded(input.tDistractOneGratingDirectionDeg));
  dist1_con_mat = unique(celleqel2mat_padded(input.tDistractOneGratingContrast));
  dist1_phase_mat = unique(celleqel2mat_padded(input.tDistractOneGratingPhaseDeg));
  dist1_az1_mat = unique(celleqel2mat_padded(input.tDistractOneGratingAzimuthDeg));
  dist1_el1_mat = unique(celleqel2mat_padded(input.tDistractOneGratingElevationDeg));
  dist1_sf1_mat = unique(celleqel2mat_padded(input.tDistractOneGratingSpatialFreqCPD)); 
  dist1_tf1_mat = unique(celleqel2mat_padded(input.tDistractOneGratingTemporalFreqCPS));
  dist1_diam1_mat = chop(unique(celleqel2mat_padded(input.tDistractOneGratingDiameterDeg)),2); 

  stim2On_mat = unique(celleqel2mat_padded(input.nStimTwoFramesOn));
  az2_mat = unique(celleqel2mat_padded(input.tTargetTwoGratingAzimuthDeg));
  el2_mat = unique(celleqel2mat_padded(input.tTargetTwoGratingElevationDeg));
  dir2_mat = unique(celleqel2mat_padded(input.tTargetTwoGratingDirectionDeg));
  diam2_mat = chop(unique(celleqel2mat_padded(input.tTargetTwoGratingDiameterDeg)),2); 
  sf2_mat = unique(celleqel2mat_padded(input.tTargetTwoGratingSpatialFreqCPD)); 
  tf2_mat = unique(celleqel2mat_padded(input.tTargetTwoGratingTemporalFreqCPS));
  con2_mat = unique(celleqel2mat_padded(input.tTargetTwoGratingContrast));
  phase2_mat = unique(celleqel2mat_padded(input.tTargetTwoGratingPhaseDeg));

  dist2_dir_mat = unique(celleqel2mat_padded(input.tDistractTwoGratingDirectionDeg));
  dist2_con_mat = unique(celleqel2mat_padded(input.tDistractTwoGratingContrast));
  dist2_phase_mat = unique(celleqel2mat_padded(input.tDistractTwoGratingPhaseDeg));
  dist2_az1_mat = unique(celleqel2mat_padded(input.tDistractTwoGratingAzimuthDeg));
  dist2_el1_mat = unique(celleqel2mat_padded(input.tDistractTwoGratingElevationDeg));
  dist2_sf1_mat = unique(celleqel2mat_padded(input.tDistractTwoGratingSpatialFreqCPD)); 
  dist2_tf1_mat = unique(celleqel2mat_padded(input.tDistractTwoGratingTemporalFreqCPS));
  dist2_diam1_mat = chop(unique(celleqel2mat_padded(input.tDistractTwoGratingDiameterDeg)),2); 
...
if tTrialN > 1
  ...
  tStr1 = ['Target One:'];
      ...
      tDistStr1 = ['Dist One:'];
      if find(dist1_con_mat > 0)
          tDistStr1 = sprintf( [tDistStr1 '\n',...
                 'Direction (deg): %s \n', ...
                 'Contrast (pct): %s \n', ...
                 'Phase (deg): %s \n'], ...
                        mat2str(dist1_dir_mat), ...
                        mat2str(dist1_con_mat.*100), ...
                        mat2str(dist1_phase_mat));
      else
        tDistStr1 = sprintf( ['No Dist One \n']);
      end

      tDistStr2 = ['Dist Two:'];
      if find(dist2_con_mat == 1)
          tDistStr2 = sprintf( [tDistStr2 '\n',...
                 'Direction (deg): %s \n', ...
                 'Contrast (pct): %s \n'], ...
                        mat2str(dist2_dir_mat), ...
                        mat2str(dist2_con_mat.*100));
      else
        tDistStr2 = sprintf( ['No Dist Two \n']);
      end
...
tV = struct('gratingDiameterDeg', []);
  tV.targetOneGratingDirectionDeg = input.targetOneGratingDirectionDeg;
  tV.targetOneGratingDiameterDeg = input.targetOneGratingDiameterDeg;
  tV.targetOneGratingAzimuthDeg = input.targetOneGratingAzimuthDeg;
  tV.targetOneGratingElevationDeg = input.targetOneGratingElevationDeg;
  tV.targetOneGratingSpatialFreqCPD = input.targetOneGratingSpatialFreqCPD;
  tV.stimOneGratingDurationMs = input.stimOneGratingOnTimeMs;
  tV.targetOneGratingTemporalFreqCPS = input.targetOneGratingTemporalFreqCPS;
  tV.targetTwoGratingDirectionDeg = input.targetTwoGratingDirectionDeg;
  tV.targetTwoGratingDiameterDeg = input.targetTwoGratingDiameterDeg;
  tV.targetTwoGratingAzimuthDeg = input.targetTwoGratingAzimuthDeg;
  tV.targetTwoGratingElevationDeg = input.targetTwoGratingElevationDeg;
  tV.targetTwoGratingSpatialFreqCPD = input.targetTwoGratingSpatialFreqCPD;
  tV.stimTwoGratingDurationMs = input.stimTwoGratingOnTimeMs;
  tV.targetTwoGratingTemporalFreqCPS = input.targetTwoGratingTemporalFreqCPS;

outStr = sprintf('Stim 1: %gms, %gdeg, %gcpd; Stim 2: %gms, %gdeg, %gcpd' , ...
                 chop(tV.stimOneGratingDurationMs,2), ...
                 tV.targetOneGratingDirectionDeg, ...
                 tV.targetOneGratingSpatialFreqCPD,...
                 chop(tV.stimTwoGratingDurationMs,2), ...
                 tV.targetTwoGratingDirectionDeg, ...
                 tV.targetTwoGratingSpatialFreqCPD);
