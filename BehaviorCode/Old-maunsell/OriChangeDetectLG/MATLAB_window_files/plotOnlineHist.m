...
%tLaserPeakMaxMw = celleqel2mat_padded(input.tLaserPeakMaxMw);
...
if isfield(input, 'laserPowerMw')
  lPowerV = subCell2PadVect(input.laserPowerMw, NaN, nTrials);
  nLP = length(chop(unique(lPowerV(~isnan(lPowerV))),4));
else
  nLP = 0;
end
if isfield(input, 'gratingDirectionDeg')
  vPowerV = subCell2PadVect(input.gratingDirectionDeg, NaN, nTrials);
...
if length(holdStarts) > 2
	...
	t2H(3) = text(0.54, 0.9, {' ', sprintf('%.0f%%', nCorr / numTrials * 100.0), ...
        ...
        if showBlock2
          ...
          elseif input.block2DoRampVTrain
            block2Str = sprintf('ramp v train: ramp %d (%s)+%d ms\n  train, pulse %d period %d dur %d', ...
        ...
        % stimulus str
        if input.doBlock2
          if input.block2DoTrialLaser & input.doVisualStim
...
if input.fixedReqHoldTimeMs+input.randReqHoldMaxMs < 1000
  ...
  maxFail = input.reactTimeMs+input.fixedReqHoldTimeMs+input.randReqHoldMaxMs;
  maxX = double(ceil((maxFail+45)./500)*500);  % round up to nearest 500 ms.
end
...
if ~isempty(input.tooFastTimeMs)
  yLim = get(gca, 'YLim');
%   xLim = input.tooFastTimeMs * [1 1];
%   fprintf(1, sprintf('%f %f %f %f %f', xLim(1), xLim(2), yLim(1), yLim(2), input.tooFastTimeMs));
  plot([input.tooFastTimeMs input.tooFastTimeMs], yLim, 'k--');
...
axH = subplot(spSz{:},6);
hold on;
holdStarts = celleqel2mat_padded(input.holdStartsMs);
...
maxReqX = double(max(input.fixedReqHoldTimeMs))+ ...
          double(max(input.randReqHoldMaxMs));
if nVisPts > 50
...
set(gca, 'XLim', [0 ceil(maxReqX/500)*500]);
set(gca, 'YLim', [0 100]);
ylabel('% of trials');
...
%   tV.gratingDirectionDeg = input.block2GratingDirectionDeg;
...
%   tV.gratingDirectionDeg = input.gratingDirectionDeg;
  tV.gratingSpeedDPS = input.gratingSpeedDPS;
end

outStr = sprintf('%gx%gdeg, at (%g,%g), %gcpd, %dms', ...
                 roundn(tV.gratingWidthDeg,-2), ...
                 roundn(tV.gratingHeightDeg,-2), ...
                 tV.gratingAzimuthDeg, ...
                 tV.gratingElevationDeg, ...
                 tV.gratingSpatialFreqCPD, ...
                 tV.gratingDurationMs);
% if tV.gratingDirectionDeg ~= 0
%   outStr = strcat(outStr, sprintf(' dir %d', tV.gratingDirectionDeg));
% end
