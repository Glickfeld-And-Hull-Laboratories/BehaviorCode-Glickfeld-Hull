function CombineAllArchData(itask)
    ...
    for imouse = 1:length(pv.mouse_mat)
        if pv.arch_mat(imouse) == 1
            ...
            if ~isempty(mouse(imouse).task(itask).pos(ipos).ind)
                            ...
                            if ~isnan(mouse(imouse).task(itask).pos(ipos).pow(ipow).expt(iexp).laser(1).thresh)
                                if ~isnan(mouse(imouse).task(itask).pos(ipos).pow(ipow).expt(iexp).laser(2).thresh)
                                    ...
                                    if itask == 1
                                        ...
                                        xval_sub(find(xval_sub<=-.1)) = chop(xval_sub(find(xval_sub<=-.1)), 1);
                                        xval_sub(find(xval_sub<.1 & xval_sub>-.1)) = roundto(xval_sub(find(xval_sub<.1 & xval_sub>-.1)),1);                                        xval_norm = xval_sub;
                                    elseif itask == 2
                                    ...
                                    mouse(imouse).task(itask).pos(ipos).pow(ipow).b2ind = [mouse(imouse).task(itask).pos(ipos).pow(ipow).b2ind cell2mat(input.tBlock2TrialNumber)];
                ...
                pn = fullfile(rc.fitOutputSummary, ['Arch-Task' num2str(itask) '-combined-summary-i' num2str(pv.mouse_mat(imouse)) '-' date '.pdf']);
