function mouse = createExptStructChr2(tasks)
        ...
        for ipos = 1:length(pv.pos_mat)
            for ipow = 1:length(pv.power_mat)
                ...
                for iexp = 1:length(mouse(imouse).task(itask).pos(ipos).pow(ipow).ind)
                    ...
                    [num2str(pv.mouse_mat(imouse)) '-' cell2mat(xd.DateStr(mouse(imouse).task(itask).pos(ipos).pow(ipow).ind(iexp)))]
                    ...
                    outS = getBehavDataForDay(...
                    'Filename', fName, ...
                    'DataIndex', cell2mat(xd.DataBlock(mouse(imouse).task(itask).pos(ipos).pow(ipow).ind(iexp))), ...
                    'WhichTrials', cell2mat(xd.TrialRangeToUse(mouse(imouse).task(itask).pos(ipos).pow(ipow).ind(iexp))), ...
                    'DoCorrectEarlies', 1, ...
                    'MergeBlock1And2', 0, ...
                    'DoBlock1Only', 0, ...
                    'SplitBlock1', 0);
                    mouse(imouse).task(itask).pos(ipos).pow(ipow).expt(iexp).outS = outS;
                ...
                if itask >= 2
                    for icon = 1:length(pv.basecon_mat)
                        ...
                        for iexp = 1:length(mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).ind)
                            ...
                            [num2str(pv.mouse_mat(imouse)) '-' cell2mat(xd.DateStr(mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).ind(iexp)))]
                            fName = fullfile(rc.pathStr,['data-i' num2str(pv.mouse_mat(imouse)) '-' cell2mat(xd.DateStr(mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).ind(iexp))) '.mat']);
                            outS = getBehavDataForDay(...
                            'Filename', fName, ...
                            'DataIndex', cell2mat(xd.DataBlock(mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).ind(iexp))), ...
                            'WhichTrials', cell2mat(xd.TrialRangeToUse(mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).ind(iexp))), ...
                            'DoCorrectEarlies', 1, ...
                            'MergeBlock1And2', 0, ...
                            'DoBlock1Only', 0, ...
                            'SplitBlock1', 0);
                            mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).expt(iexp).outS = outS;
                            ...
                            if length(fitS) == 2
                                ...
                                if fitS(1).coefEsts(3)>=.9
                                    if fitS(2).coefEsts(3)>=.9
                                        mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).expt(iexp).miss = 1;
                                    else
                                        mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).expt(iexp).miss = 0;
                                    end
                                else
                                    mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).expt(iexp).miss = 0;
                                end
                ...
                for iexp = 1:length(mouse(imouse).task(itask).pos(ipos).pow(ipow).ind)
                    if mouse(imouse).task(itask).pos(ipos).pow(ipow).expt(iexp).laser(1).good == 1;
                        fn = fullfile(rc.pathStr, ['data-i' num2str(pv.mouse_mat(imouse)) '-' num2str(cell2mat(xd.DateStr(mouse(imouse).task(itask).pos(ipos).pow(ipow).ind(iexp)))) '.mat']);
                        load(fn)
                    ...
                    for icon = 1:length(pv.basecon_mat)
                        for iexp = 1:length(mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).ind)
                            if mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).expt(iexp).laser(1).good == 1;
                                fn = fullfile(rc.pathStr, ['data-i' num2str(pv.mouse_mat(imouse)) '-' num2str(cell2mat(xd.DateStr(mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).ind(iexp)))) '.mat']);
                                load(fn)
                                ...
                                early_pct = sum([length(block1_ind_early) length(block2_ind_early)])./length(range);
                                if early_pct<0.5
                                    mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).expt(iexp).early = 1;
                                else
                                    mouse(imouse).task(itask).pos(ipos).pow(ipow).con(icon).expt(iexp).early = 0;
                                end
                            end
                        end
                    end
