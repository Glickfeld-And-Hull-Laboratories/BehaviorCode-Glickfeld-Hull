function mouse = createExptStructAVCtrl;
rc = behavConstsAV;
...
for imouse = 1:nMice;
    ...
    for iexp = 1:length(ind)
        ...
        if ~isnan(eval(cell2mat(xd.ChooseMatFile(ind(iexp)))))
            n = n(eval(cell2mat(xd.ChooseMatFile(ind(iexp)))));
        end
...
%             mouse(imouse).input(iexp).hitrates = zeros(1,length(mouse(imouse).input(iexp).uniqueDeg));
            ...
            mouse(imouse).input(iexp).isBlock2 = input_temp.tBlock2TrialNumber;
