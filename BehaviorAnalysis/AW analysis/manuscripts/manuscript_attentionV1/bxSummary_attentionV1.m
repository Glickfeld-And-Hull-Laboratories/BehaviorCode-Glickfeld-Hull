...
cycle_edges = [2 3 4 5 6 11];
cyc_label = {'2','3','4','5','6-10'};
...
%         end
        ...
        valReact = bxExp(iexp).valReact;
        invReact = bxExp(iexp).invReact;
        
        if any(strcmp(attnMice,num2str(sn)))
            ...
            valReactTime_all = cat(2,valReactTime_all,valReact);
            invReactTime_all = cat(2,invReactTime_all,invReact);
        end
        ...
        if isfield(ms,'visTargets')           
            ...
            ms(sn_ind).valReact = cat(2,ms(sn_ind).valReact,valReact);
            ms(sn_ind).invReact = cat(2,ms(sn_ind).invReact,invReact);
            ...
            ms(sn_ind).valReact = valReact;
            ms(sn_ind).invReact = invReact;
        end
...
if doImgData
    ...
    %         end
            ...
            valReact = bxImgExp(iexp).valReact;
            invReact = bxImgExp(iexp).invReact;
...
for im = 1:nmice
    ...
    visFit = weibullFitLG(...
        msSummary(im).av(visualTrials).binning(oriBin).cue(1).binTarget,...
        msSummary(im).av(visualTrials).binning(oriBin).cue(1).hitRate,...
        1,0,{'nTrials',...
        msSummary(im).av(visualTrials).binning(oriBin).cue(1).nTrials});
    audFit = weibullFitLG(...
        msSummary(im).av(auditoryTrials).binning(oriBin).cue(1).binTarget,...
        msSummary(im).av(auditoryTrials).binning(oriBin).cue(1).hitRate,...
        1,0,{'nTrials',...
        msSummary(im).av(auditoryTrials).binning(oriBin).cue(1).nTrials});
    ...
    vline(visFit.thresh,'k--',num2str(visFit.thresh))
    ax = gca;
    ax.XScale = 'log';
    figXAxis([],'Orientation Change (deg)',visLevels_lim,visLevels_label,visLevels_label)
    figYAxis([],'Hit Rate (%)',HR_lim,HR_label,HR_label)
    figAxForm
    title(sprintf('Visual Trials, attn: p = %s', num2str(msSummary(im).av...
        (visualTrials).belowThreshAttnTestP)))
    ...
    vline(audFit.thresh,'k--',num2str(audFit.thresh))
    ax = gca;
    ax.XScale = 'log';
    ...
    figAxForm
    title(sprintf('Visual Trials, attn: p = %s', num2str(msSummary(im).av...
        (auditoryTrials).belowThreshAttnTestP)))
...
%%
visInvCutoff = invVisBinCutoff+1;
msMatchedHR = struct;
for im = 1:nmice
...
visTargets = unique(visTargets_all(visTrials));
audTargets = unique(audTargets_all(audTrials));
...
invVisBinEdges = [min(visTargets)-1 invVisBinCutoff+1 max(visTargets)];
invAudBinEdges = [min(audTargets)-(0.5*min(audTargets)) ...
    invAudBinCutoff max(audTargets)];
[~,~,invVisBinInd] = histcounts(invVisTargets_all,invVisBinEdges);
[~,~,invAudBinInd] = histcounts(invAudTargets_all,invAudBinEdges);
...
nInvVisHits = nan(1,2);
nInvVisMisses = nan(1,2);
nInvAudHits = nan(1,2);
nInvAudMisses = nan(1,2);
invVisTargetsBinned = nan(1,2);
invAudTargetsBinned = nan(1,2);
invVisTargetsSte = nan(1,2);
invAudTargetsSte = nan(1,2);
for ibin = 1:2
...
figure
suptitle('Summary Across All Mice with Attn, Imaging and Training Data')
subplot 121
...
h.MarkerFaceColor = [1 1 1];
vline(invVisBinCutoff,'k--','inv bin cutoff')
ax = gca;
ax.XScale = 'log';
figXAxis([],'Orientation Change (deg)',visLevels_lim,visLevels_label,visLevels_label)
figYAxis([],'Hit Rate (%)',HR_lim,HR_label,HR_label)
...
h.Color = cueColors{inv};
h.MarkerFaceColor = [1 1 1];
vline(invAudBinCutoff,'k--','inv bin cutoff')
ax = gca;
ax.XScale = 'log';
...
print(fullfile(fnout,'HR_allMice'),'-dpdf','-fillpage')

%% across mice time bin

[~,~,valCycBinInd] = histcounts(nCycles_all,cycle_edges);
[~,~,invCycBinInd] = histcounts(invCycles_all,cycle_edges);

nCyc = length(cycle_edges)-1;

nValVisHitsCyc = nan(1,nCyc);
nValVisMissCyc = nan(1,nCyc);
nInvVisHitsCyc = nan(1,nCyc);
nInvVisMissCyc = nan(1,nCyc);

nValAudHitsCyc = nan(1,nCyc);
nValAudMissCyc = nan(1,nCyc);
nInvAudHitsCyc = nan(1,nCyc);
nInvAudMissCyc = nan(1,nCyc);

for icyc = 1:nCyc
    invInd = invCycBinInd == icyc & invVisTrials;
    nInvVisHitsCyc(icyc) = sum(invHit(invInd));
    nInvVisMissCyc(icyc) = sum(invMiss(invInd));
    
    valInd = valCycBinInd == icyc & visTrials;
    matches = cell2mat(getMatchedValidTrialIndex(visTargets_all(valInd),...
        invVisTargets_all(invInd)));
    tempHit = hit(valInd);
    tempMiss = miss(valInd);
    
    nValVisHitsCyc(icyc) = sum(tempHit(matches));
    nValVisMissCyc(icyc) = sum(tempMiss(matches));
    
    invInd = invCycBinInd == icyc & invAudTrials;
    nInvAudHitsCyc(icyc) = sum(invHit(invInd));
    nInvAudMissCyc(icyc) = sum(invMiss(invInd));
    
    valInd = valCycBinInd == icyc & audTrials;
    matches = cell2mat(getMatchedValidTrialIndex(audTargets_all(valInd),...
        invAudTargets_all(invInd)));
    tempHit = hit(valInd);
    tempMiss = miss(valInd);
    
    nValAudHitsCyc(icyc) = sum(tempHit(matches));
    nValAudMissCyc(icyc) = sum(tempMiss(matches));
end
% visInd = ~isnan(nValVisHitsCyc);
% invVisInd = ~isnan(nInvVisHitsCyc); 
% audInd = ~isnan(nAudHits);
% invAudInd = ~isnan(nInvAudHits);

[valVisCycHR,valVisCycCI95] = binofit(nValVisHitsCyc,...
    nValVisHitsCyc+nValVisMissCyc);
[invVisCycHR,invVisCycCI95] = binofit(nInvVisHitsCyc,...
    nInvVisHitsCyc+nInvVisMissCyc);

[valAudCycHR,valAudCycCI95] = binofit(nValAudHitsCyc,...
    nValAudHitsCyc+nValAudMissCyc);
[invAudCycHR,invAudCycCI95] = binofit(nInvAudHitsCyc,...
    nInvAudHitsCyc+nInvAudMissCyc);

figure;
suptitle('Bins Matched for Difficulty, No Minimum Trial Number')
subplot 121
x = 1:nCyc;
y = valVisCycHR.*100;
yErrL = y - (valVisCycCI95(:,1)'.*100);
yErrU = (valVisCycCI95(:,2)'.*100) - y;
h = errorbar(x,y,yErrL,yErrU,'o');
h.Color = cueColors{val};
h.MarkerFaceColor = [1 1 1];
hold on
y = invVisCycHR.*100;
yErrL = y - (invVisCycCI95(:,1)'.*100);
yErrU = (invVisCycCI95(:,2)'.*100) - y;
h = errorbar(x,y,yErrL,yErrU,'o');
h.Color = cueColors{inv};
h.MarkerFaceColor = [1 1 1];
figXAxis([],'Stimulus Number',[0 nCyc+1],x,cyc_label);
figYAxis([],'Hit Rate (%)',HR_lim,HR_label,HR_label);
figAxForm
title('Visual Trials')
...
x = 1:nCyc;
y = valAudCycHR.*100;
yErrL = y - (valAudCycCI95(:,1)'.*100);
yErrU = (valAudCycCI95(:,2)'.*100) - y;
h = errorbar(x,y,yErrL,yErrU,'o');
h.Color = cueColors{val};
h.MarkerFaceColor = [1 1 1];
hold on
y = invAudCycHR.*100;
yErrL = y - (invAudCycCI95(:,1)'.*100);
yErrU = (invAudCycCI95(:,2)'.*100) - y;
h = errorbar(x,y,yErrL,yErrU,'o');
h.Color = cueColors{inv};
h.MarkerFaceColor = [1 1 1];
figXAxis([],'Stimulus Number',[0 nCyc+1],x,cyc_label);
...
figAxForm
title('Auditory Trials')
print(fullfile(fnout,'HRbyTrLength_allMice'),'-dpdf','-fillpage')
%% across mice time bin
...
for icyc = 1:nCyc
    invInd = invCycBinInd == icyc & invVisTrials & ...
        invVisTargets_all <= invVisBinCutoff;
    nInvVisHitsCyc(icyc) = sum(invHit(invInd));
    nInvVisMissCyc(icyc) = sum(invMiss(invInd));
    
    valInd = valCycBinInd == icyc & visTrials & ...
        visTargets_all <= invVisBinCutoff;
    matches = cell2mat(getMatchedValidTrialIndex(visTargets_all(valInd),...
        invVisTargets_all(invInd)));
    tempHit = hit(valInd);
    tempMiss = miss(valInd);
    ...
    invInd = invCycBinInd == icyc & invAudTrials &...
        invAudTargets_all <= invAudBinCutoff;
    nInvAudHitsCyc(icyc) = sum(invHit(invInd));
    nInvAudMissCyc(icyc) = sum(invMiss(invInd));
...
figure;
suptitle('Hard Trials, No Minimum Trial Number')
subplot 121
x = 1:nCyc;
y = valVisCycHR.*100;
yErrL = y - (valVisCycCI95(:,1)'.*100);
...
figAxForm
title('Auditory Trials')
print(fullfile(fnout,'HRbyTrLengthHarTrials_allMice'),'-dpdf','-fillpage')
