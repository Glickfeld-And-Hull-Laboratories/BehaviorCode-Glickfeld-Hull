...
figAxForm
print(fullfile(fnout,'FAR_LR_allMice'),'-dpdf','-fillpage')
...
figAxForm
title('Auditory Trials')
print(fullfile(fnout,'RT_allMice'),'-dpdf','-fillpage')
...
for im = 1:2
    for iav = 1:2
        ...
        figYAxis([],'Norm. Hit Rate',[0 1],HR_label./100,HR_label./100)
        figAxForm
        if im == 2
...
for im = 1:nMice
    ...
    visTargets = unique(cat(2,visTargets,msHR(im).av(visualTrials).cue(valid).targets));
    audTargets = unique(cat(2,audTargets,msHR(im).av(auditoryTrials).cue(valid).targets));
