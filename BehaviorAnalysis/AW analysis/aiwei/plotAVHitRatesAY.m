...
for imouse = 1:size(av,2);
    ...
    reactC =  celleqel2mat_padded(input.leverUpTimeMs) -( celleqel2mat_padded(input.leverDownTimeMs) +((unique(input.stimOnTimeMs)+unique(input.stimOffTimeMs)).*celleqel2mat_padded(input.catchCyclesOn)));
    reactCsuccess = reactC(successIx);
    targetsuccess = targetDirectionDeg(successIx);
    catchFA = catchDirectionDeg(FAIx);
    atargetsuccess = targetAmplitude(successIx);
    acatchFA = catchAmplitude(FAIx);
    ...
    for ibin = 1:max(amp_bins,[],2)
        ...
        if length(ind)>5%whyfive?
            ...
            reacttime_amp(ibin) = mean(reactVsuccess(ind),2);
            reacttime_sem_amp(ibin) = std(reactVsuccess(ind),[],2)/sqrt(length(reactVsuccess(ind)));
        end
        ...
        if length(indc)>5
            ...
            reacttime_ampc(ibin) = mean(reactCsuccess(indc),2);
            reacttime_sem_ampc(ibin) = std(reactCsuccess(indc),[],2)/sqrt(length(reactCsuccess(indc)));
        end
    end
...
%     [FR_amp, ci95_FR_amp] = binofit(FAs_amp, CRs_amp + FAs_amp);
    ...
    for ibin = 1:max(ori_bins,[],2)
        ...
        if length(ind)>5
            ...
            reacttime_ori(ibin) = mean(reactVsuccess(ind),2);
            reacttime_sem_ori(ibin) = std(reactVsuccess(ind),[],2)/sqrt(length(reactV(ind)));
        end
        ...
        if length(indc)>5
            ...
            reacttime_oric(ibin) = mean(reactCsuccess(indc),2);
            reacttime_sem_oric(ibin) = std(reactCsuccess(indc),[],2)/sqrt(length(reactC(indc)));
        end
    end
...
%     errorbarxy(avg_oric, FR_ori, sem_oric, sem_oric, FR_ori - ci95_FR_ori(:,1)', ci95_FR_ori(:,2)' - FR_ori, {'oc', 'c', 'c'});
    ...
    ylabel('React Time')
    ylim([-200 max(reacttime_oric) + 200])
    xlim([0 100])
    ...
    for ibin = 1:length(ind_catch)
       ...
       reactamp(ibin) = mean(reactVsuccess(indT),2);
       reactampc(ibin) =  mean(reactCsuccess(indC),2);
       react_sem_amp(ibin) = std(reactVsuccess(indT),[],2)/sqrt(length(reactVsuccess(indT)));
       react_sem_ampc(ibin) = std(reactCsuccess(indC),[],2)/sqrt(length(reactCsuccess(indC)));
...
%     ylim([0 1])
    ...
    for ibin = 1:length(ind_catch)
       ...
       reactori(ibin) = mean(reactVsuccess(indT),2);
       reactoric(ibin) =  mean(reactCsuccess(indC),2);
       react_sem_ori(ibin) = std(reactVsuccess(indT),[],2)/sqrt(length(reactVsuccess(indT)));
       react_sem_oric(ibin) = std(reactCsuccess(indC),[],2)/sqrt(length(reactCsuccess(indC)));
...
%     xlim([0 300])
%     ylim([0 300])
     xlim([0 max(reactoric)+200])
     ylim([0 max(reactoric)+200])
    hold on
