...
rootDir = 'Z:\Analysis\Behavior_data';
rc.indexFilename = fullfile(rootDir, 'experimentIndexes\subj-days-lg.xls');
rc.indexcatchFilename = fullfile(rootDir, 'experimentIndexes\catch-days.xlsx');
...
%graph1 auditory catch
    ...
    for i = 1:length(dirs)-1
        ...
        audHitRate(i) = sum(hitInd(trials))/(sum(hitInd(trials))+sum(missInd(trials)));
        trialN(i) = sum(hitInd(trials))+sum(missInd(trials));
    end

    for i = 1:length(catchDeg)-1
        trials = find(doCatch == catchDeg(i+1));
        audcatchHitRate(i) = sum(catchYesInd(trials))/(sum(catchTrial(trials)));
    ...
    audHitRateCI = prctile(visHitRateBoots,[2.5 97.5],2);
    ...
    audcatchHitRateCI = prctile(catchHitRateBoots,[2.5 97.5],2);
    ...
    figure;
    errorbar(dirs(2:end),audHitRate,audHitRate-audHitRateCI(:,1)',audHitRateCI(:,2)'-audHitRate,'go','LineStyle', 'none');
    trialNLabel = strcat(repmat({'\leftarrow'}, [length(trialN) 1]),cellstr(num2str(trialN(:))), repmat({' trials'}, [length(trialN) 1]))';
    text(dirs(2:end),audHitRate,trialNLabel)
    ...
    % hold on
    errorbar(catchDeg(2:end),audcatchHitRate,audcatchHitRate-audcatchHitRateCI(:,1)',audcatchHitRateCI(:,2)'-audcatchHitRate,'ko','LineStyle','none');
    ...
    text(catchDeg(2:end),audcatchHitRate,catchTrialNLabel)
...
figure;
errorbar(dirs(2:end),visHitRate1,visHitRate1-visHitRateCI(:,1)',visHitRateCI(:,2)'-visHitRate1,'go','LineStyle', 'none');
