               ...
               qvals_temp_mean=nanmean(qvals_adj,2);  % mean qVals of trials that share same conditions
                   for itrial=1:length(temp)
                      if iloc==1 && ibehavior==2 %right correct
                          temptime1 =min(find(qvals_adj(8000:18000,itrial)<=rdt)); % removed (itrial) after temptime1 - CLM
                          if isempty(temptime1)
                              if isempty(min(find(qvals_adj(8000:18000)>=ldt)))
                                  continue
                              else
                                temptime(itrial)=min(find(qvals_adj(8000:18000)>=ldt)); end
                          else
                              temptime(itrial)=temptime1; end       
                      elseif iloc==1 && ibehavior==1 %right incorrect
                          temptime1 =min(find(qvals_adj(8000:18000,itrial)>=ldt)); 
                          if isempty(temptime1)
                              if isempty(min(find(qvals_adj(8000:18000)<=rdt)))
                                  continue
                              else
                              temptime(itrial)=min(find(qvals_adj(8000:18000)<=rdt)); end
                          else
                              temptime(itrial)=temptime1; end                           
                      ...
                      elseif iloc==2 && ibehavior==1 %left incorrect
                          temptime1 =min(find(qvals_adj(8000:18000,itrial)<=rdt));
                          if isempty(temptime1)
                              if isempty(min(find(qvals_adj(8000:18000)>=ldt)))
                                  continue
                              else
...
for icontrol=1:nledcond  
    for iloc=1:nstimlocation    
        for irat=1:nratios            
          ...
          % just use rxttime_avg to segregate...
%            if iloc==1               
...
% What is variable ndatarun? 
% Add code for mean and SEM here...
