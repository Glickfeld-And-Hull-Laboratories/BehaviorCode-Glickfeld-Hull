...
PV_Chr2 = [{'i419'}; {'i548'}; {'i565'}; {'i578'}; {'i593'}];
SOM_Chr2 = [{'i414'}; {'i461'}; {'i581'}; {'i582'}];
PV_Arch = [{'i441'}; {'i442'}; {'i443'}];
SOM_Arch = [{'i459'}; {'i460'}; {'i462'}; {'i463'}];
Control = [{'i591'}; {'i592'}];

cohorts = {PV_Chr2, SOM_Chr2, PV_Arch, SOM_Arch, Control};
cohort_names = [{'PV_Chr2'}; {'SOM_Chr2'}; {'PV_Arch'}; {'SOM_Arch'}; {'Control'}];
mice = [];
...
for c = 1:length(cohorts)
    ...
    %    tempStruct(m).tDecisionTimeMs = input.tD ecisionTimeMs;
...
%     quad_data(m).mscLED_right_SEM = mscLED_right_SEM;
    ...
    mouse_quad_data = quad_data(m);
    
    save(['\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\camaron\Analysis\Contrast_discrim\data\final\'...
         mouse '_quad_info.mat'], 'mouse_quad_data');


    clearvars -except allmice data_path mouse_names_PV_contrast quad_data PV_Chr2 SOM_Chr2 PV_Arch SOM_Arch SOM_Arch...
        cohorts cohort_names c m mice

    end

%     save(['\\duhs-user-nc1.dhe.duke.edu\dusom_glickfeldlab\All_Staff\home\camaron\Analysis\Contrast_discrim\data\final\'...
%          cohort_names{c} '_quad_info.mat'], 'quad_data');
end
